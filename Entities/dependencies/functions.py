...
class P:
    ...
    @property
    def pdb_header(self) -> str:
        return self.__str.replace("] ", f']{Fore.MAGENTA} PDB ')
    ...
    def __init__(self, value: object, *, color:Literal['white', 'red', 'blue', 'green', 'cyan', 'yellow', 'magenta', 'nenhum'] = "nenhum", title:Literal["", "REPORT", "ERROR", "COMPLETED"]="") -> None:
        ...
        self.__str = f"{self.date}{(f"{self.__title} - " if self.__title else "- ")}"
        if color == 'nenhum':
            self.__str += f"{self.__value + Fore.RESET}"
        else:
            self.__str += f"{self.color + self.__value + Fore.RESET}"
